        ...
        private static List<ClienteDTO> RealizarConsultaClientes(String consulta)
        {
            ...
            try
            {
                ...
                while (dataReader.Read())
                {
                    ...
                    ClienteDTO cliente = new ClienteDTO(Convert.ToString(dataReader["CustomerID"]), Convert.ToString(dataReader["CompanyName"]));
        ...
        private static List<PedidoDTO> RealizarConsultaPedidos(String consulta)
        {
            ...
            try
            {
                ...
                while (dataReader.Read())
                {
                    ...
                    PedidoDTO pedido = new PedidoDTO(Convert.ToInt32(dataReader["OrderID"]), Convert.ToString(dataReader["CustomerID"]),
                        Convert.ToDateTime(dataReader["OrderDate"]), Convert.ToDateTime(dataReader["ShippedDate"]), Convert.ToDouble(dataReader["Freight"]),
                        Convert.ToString(dataReader["ShipAddress"]));
